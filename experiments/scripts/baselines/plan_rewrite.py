...
Focus on:
1. Adding more detail to algorithmic steps.
2. Covering more edge cases.
...
Focus on:
1. Ensuring the code perfectly matches the implementation plan.
...
def process_problem(
    ...
    """Process a single problem with iterative plan refinement and code refinement."""
    ...
    p_console.print("\n[bold cyan]Step 1: Generating Initial Plan...[/bold cyan]")
    plan_prompt = PLAN_PROMPT_TEMPLATE.format(
        question_content=problem.question_content,
        starter_code=problem.starter_code,
    )
    try:
        current_plan = generate(plan_prompt)
        plans.append(current_plan)
        p_console.print(Panel(Syntax(current_plan, "markdown", theme="monokai", padding=1), title="Initial Plan"))
    except Exception as e:
        p_console.print(f"[bold red]Failed initial plan generation: {e}[/bold red]")
        return (problem.question_id, False, log_stream.getvalue(), snippets, plans)

    # Step 2: Plan Refinement Loop
    for i in range(plan_iterations):
        p_console.print(f"\n[bold cyan]Step 2.{i+1}: Refining Plan ({i+1}/{plan_iterations})...[/bold cyan]")
        refine_plan_prompt = PLAN_REFINEMENT_PROMPT_TEMPLATE.format(
            question_content=problem.question_content,
            starter_code=problem.starter_code,
            previous_plan=current_plan,
        )
        try:
            current_plan = generate(refine_plan_prompt)
            plans.append(current_plan)
            p_console.print(Panel(Syntax(current_plan, "markdown", theme="monokai", padding=1), title=f"Refined Plan {i+1}"))
        except Exception as e:
            p_console.print(f"[bold red]Failed plan refinement {i+1}: {e}[/bold red]")
            break

    # Step 3: Code Generation from final plan
    p_console.print("\n[bold cyan]Step 3: Generating Code from Final Plan...[/bold cyan]")
    ...
    try:
        res_code = generate(solution_prompt)
        ...
        p_console.print(f"[bold red]Failed code generation: {e}[/bold red]")
        return (problem.question_id, False, log_stream.getvalue(), snippets, plans)

    # Step 4: Final Code Refinement
    p_console.print("\n[bold cyan]Step 4: Refining Final Code...[/bold cyan]")
    code_refine_prompt = CODE_REFINEMENT_PROMPT_TEMPLATE.format(
        question_content=problem.question_content,
        plan=current_plan,
        starter_code=problem.starter_code,
    ...
    try:
        res_refine = generate(code_refine_prompt)
        ...
        p_console.print(f"[bold red]Failed final code refinement: {e}[/bold red]")

    status_str = "[bold green]GENERATED[/bold green]" if len(snippets) > 0 else "[bold red]FAILED[/bold red]"
...
def run(
    ...
    output_dir: Path = typer.Option(Path("logs/plan_rewrite"), help="Directory to save logs"),
    ...
    log_file = output_dir / f"{run_id}_plan_rewrite.txt"
    ...
    console.print(
        Panel(
            f"Starting Plan-Rewrite Code Generation Run: [bold cyan]{run_id}[/bold cyan]\n"
    ...
    with Progress(
        ...
        overall_task = progress.add_task("[cyan]Processing problems with plan-rewrite...", total=total_problems)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for fut in future_to_problem:
                try:
                    q_id, passed, log_block, snippets, plans = fut.result()
    ...
    # Final result table
    table = Table(title="Plan-Rewrite Generation Results")
