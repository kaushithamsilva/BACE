...
def process_problem(
    ...
    """Process a single problem with example-free modification and iterative refinement."""
...
def run(
    ...
    output_dir: Path = typer.Option(Path("logs/no_public_plan_rewrite"), help="Directory to save logs"),
    ...
    log_file = output_dir / f"{run_id}_no_public_plan_rewrite.txt"
    ...
    console.print(
        Panel(
            f"Starting No-Public-Plan-Rewrite Run: [bold cyan]{run_id}[/bold cyan]\n"
    ...
    with Progress(
        ...
        overall_task = progress.add_task("[cyan]Processing no-public-plan-rewrite workflow...", total=total_problems)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for fut in future_to_problem:
                try:
                    q_id, passed, log_block, snippet, modified_prob, all_snippets, all_plans = fut.result()
    ...
    # Final result table
    table = Table(title="No-Public-Plan-Rewrite Results")
