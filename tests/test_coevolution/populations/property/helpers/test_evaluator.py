...
def test_property_eval_worker_sorting_and_early_break():
    ...
    import json
    pairs = [
        {"input_arg": json.dumps({"input_arg": "long_input_data_string"}), "output": json.dumps("output1")},
        {"input_arg": json.dumps({"input_arg": "short"}), "output": json.dumps("output2")},
        {"input_arg": json.dumps({"input_arg": "medium_input"}), "output": json.dumps("output3")},
    ]

    # Sort them as the evaluator would before passing to worker
    sorted_pairs = sorted(pairs, key=lambda p: len(str(p["input_arg"])))
    assert "short" in sorted_pairs[0]["input_arg"]
    assert "medium_input" in sorted_pairs[1]["input_arg"]
    assert "long_input_data_string" in sorted_pairs[2]["input_arg"]

    # Mock execute_code to fail on the SECOND shortest input ("medium_input")
    def side_effect(script, runtime):
        # We check if the script contains the medium_input string
        # compose_property_test_script now uses repr(input_arg) and repr(output)
